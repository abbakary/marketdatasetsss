...
const initialWishlistIds = ['d1', 'd4', 'd8', 'd2'];
...
export default function WishlistPage() {
  ...
  const wishlistDatasets = allDatasets.filter(d => wishlistIds.includes(d.id));
  const filtered = wishlistDatasets.filter(d => d.title.toLowerCase().includes(searchQuery.toLowerCase()));
  const totalValue = wishlistDatasets.reduce((s, d) => s + parseFloat(d.price), 0);

  const handleRemove = () => {
    if (!itemToRemove) return;
    setWishlistIds(prev => prev.filter(id => id !== itemToRemove));
