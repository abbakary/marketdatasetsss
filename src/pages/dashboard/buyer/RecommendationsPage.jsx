...
export default function RecommendationsPage() {
  ...
  const filtered = selectedCategory === 'All' ? approved : approved.filter(d => d.category === selectedCategory);

  const toggleWishlist = id => { setWishlist(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]); showToast(wishlist.includes(id) ? 'Removed from wishlist' : 'Added to wishlist'); };
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }}>
          <div style={{ background: '#fff', borderRadius: 28, padding: 32, width: '100%', maxWidth: 580, boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', border: '1px solid #e2e8f0', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 24, borderTop: '1px solid #edf2f7' }}>
              <div>
                <p style={{ fontSize: 12, color: '#a0aec0', margin: '0 0 2px', fontWeight: 700, textTransform: 'uppercase' }}>Instant Access</p>
                <span style={{ fontSize: 32, fontWeight: 900, color: '#20B2AA' }}>${selected.price}</span>
              </div>
              <div style={{ display: 'flex', gap: 12 }}>
                <button onClick={() => toggleWishlist(selected.id)} style={{ padding: '14px 20px', background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, color: '#e53e3e', cursor: 'pointer', fontSize: 15, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Heart size={18} fill={wishlist.includes(selected.id) ? '#e53e3e' : 'none'} /> {wishlist.includes(selected.id) ? 'Saved' : 'Save'}
                </button>
                <button onClick={() => { addToCart(selected.id); setSelected(null); }} style={{ padding: '14px 28px', background: '#FF8C00', border: 'none', borderRadius: 16, color: '#fff', cursor: 'pointer', fontSize: 15, fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8, boxShadow: '0 4px 12px rgba(255,140,0,0.3)' }}>
