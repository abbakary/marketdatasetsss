...
export default function BudgetTrackerPage() {
  ...
  const affordableDatasets = allDatasets.filter(d => d.status === 'approved' && parseFloat(d.price) <= remaining).slice(0, 4);
    ...
    <DashboardLayout role="buyer">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <ChartCard title={`Datasets Within Your Remaining Budget ($${remaining.toLocaleString()})`}>
          ...
          {affordableDatasets.length === 0 && <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.5)', padding: '24px 0' }}>No datasets within remaining budget</p>}
        </ChartCard>
      </div>
